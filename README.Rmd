...
- A "significance" criterion and threshold for the inference model. For instance, the t-test in a linear regression has its p-value as the "significance" criterion, and a common threshold for statistical significance is 0.05. 
...
Since we want to create a power curve, we need to supply a list of outcome models with different effect sizes:
...
Here are examples of how to use other inference models included in the package.
...
```{r bws-test, results='hide', fig.keep='all'}
...
```{r qg-test, results='hide', fig.keep='all'}
...
```{r fin-test, results='hide', fig.keep='all'}
...
```{r bma-test, results='hide', fig.keep='all'}
...
```{r bkmr-test, results='hide', fig.keep='all'}
...
```{r, include=F, eval=F}
ms_imod <- InferenceModel(model = "ms", nrun = 100, verbose=F)
ms_power <- sim_power(xmod, ymod, ms_imod, s = 2, n = 100, 
                       cores=3, snr_iter=100, errorhandling = "stop")
...
We show how to work with binary outcome here. See section 5 in our paper for more details. Data can be downloaded at: ttps://ehjournal.biomedcentral.com/articles/10.1186/s12940-020-00642-6
